...
app.get('/foods' , (req, res) => {
    ...
    client.connect(err => {
        if(err){
            ...
            const collection = client.db('hotOnionRestaurent').collection('foods');
...
app.get('/features' , (req,res) => {
    client = new MongoClient(uri , {useNewUrlParser:true , useUnifiedTopology: true});
    client.connect(err => {
        const collection = client.db('hotOnionRestaurent').collection('features');
        collection.find().toArray((rej,documents) => {
            if(rej){
...
app.post('/submitorder' , (req,res) => {
    ...
    client = new MongoClient(uri , {useNewUrlParser:true , useUnifiedTopology: true});
    client.connect(err => {
        const collection = client.db('hotOnionRestaurent').collection('orders');
